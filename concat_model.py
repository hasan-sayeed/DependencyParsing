...
class MODEL_concat(nn.Module):
    def __init__(self, pos_vocab_size, pos_embedding_dim, stack_buff_dim, hidden_dim, class_size):
        ...
        self.linear_pos = nn.Linear(4*pos_embedding_dim, hidden_dim, bias = True)
    ...
    def forward(self, feature_stack_buff, feature_pos):
        ...
        embedded_feature_pos = torch.flatten(embedded_feature_pos, start_dim = 1)
...
feature_stack_buff_train, feature_pos_train, target_train = process_data(file_path = train_file_path, path_to_pos_set = 'data/pos_set.txt', path_to_tag_set = 'data/tagset.txt', name_glove = name_glove, dim_glove = dim_glove, rep_type = 'concat')
feature_stack_buff_dev, feature_pos_dev, target_dev = process_data(file_path = dev_file_path, path_to_pos_set = 'data/pos_set.txt', path_to_tag_set = 'data/tagset.txt', name_glove = name_glove, dim_glove = dim_glove, rep_type = 'concat')
feature_stack_buff_test, feature_pos_test, target_test = process_data(file_path = test_file_path, path_to_pos_set = 'data/pos_set.txt', path_to_tag_set = 'data/tagset.txt', name_glove = name_glove, dim_glove = dim_glove, rep_type = 'concat')
...
for learning_rate in learning_rates:
    ...
    model = MODEL_concat(pos_vocab_size = number_of_pos, pos_embedding_dim = 50, stack_buff_dim = feature_stack_buff_train.shape[1], hidden_dim = 200, class_size = number_of_classes).to(device)
    ...
    for ep in range(1, max_epochs+1):
        ...
        with torch.no_grad():  # Disable gradient computation during validation
            for stack_buff, pos, lab in dev_loader:
                ...
                predicted_actions = act_pred(model, file_path=dev_file_path, cwindow=2, name_glove = name_glove, dim_glove = dim_glove, rep_type = 'concat')
                print(gold_actions)
